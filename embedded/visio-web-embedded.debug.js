...
(function (OfficeExtension_1) {
	...
	var HostBridge=(function () {
		...
		HostBridge.init=function (bridge) {
			...
			HttpUtility.setCustomSendLocalDocumentRequestFunc(function (request) {
				...
				return instance.sendMessageToHostAndExpectResponse(bridgeMessage)
		...
		HostBridge.prototype.removeHostMessageHandler=function (handler) {
			var index=HostBridge.s_instance.m_handlers.indexOf(handler);
			if (index >=0) {
				HostBridge.s_instance.m_handlers.splice(index);
			}
		};
	...
	var CoreUtility=(function () {
		...
		CoreUtility._parseErrorResponse=function (responseInfo) {
			...
			if (!CoreUtility.isNullOrUndefined(errorObj) && typeof (errorObj)==="object" && errorObj.error) {
				...
				errorMessage=CoreUtility._getResourceString(CoreResourceStrings.connectionFailureWithDetails, [responseInfo.statusCode.toString(), errorObj.error.code, errorObj.error.message]);
			}
			else {
	...
	var ClientRequestBase=(function () {
		...
		ClientRequestBase.prototype.buildRequestMessageBodyAndRequestFlags=function () {
			...
			for (var index=0; index < this.m_actions.length; index++) {
				...
				if (action.actionInfo.ActionType===3 && action.actionInfo.Name===CommonConstants.keepReference) {
		...
		ClientRequestBase.prototype._removeKeepReferenceAction=function (objectPathId) {
			for (var i=this.m_actions.length - 1; i >=0; i--) {
				...
				if (actionInfo.ObjectPathId===objectPathId && actionInfo.ActionType===3 && actionInfo.Name===CommonConstants.keepReference) {
	...
	var ServiceApiQueue=(function () {
		...
		ServiceApiQueue.prototype.processActions=function () {
			...
			executor.executeAsync(this.m_context._customData, flags, requestMessage)
	...
	var HttpRequestExecutor=(function () {
		...
		HttpRequestExecutor.prototype.executeAsync=function (customData, requestFlags, requestMessage) {
			...
			var sendRequestFunc=CoreUtility._isLocalDocumentUrl(requestInfo.url) ? HttpUtility.sendLocalDocumentRequest : HttpUtility.sendRequest;
			return sendRequestFunc(requestInfo)
	...
	var CommonUtility=(function (_super) {
		...
		CommonUtility.throwIfApiNotSupported=function (apiFullName, apiSetName, apiSetVersion, hostName) {
			...
			if (!CommonUtility.isSetSupported(apiSetName, apiSetVersion)) {
				var message=CoreUtility._getResourceString(CoreResourceStrings.apiNotFoundDetails, [apiFullName, apiSetName+" "+apiSetVersion, hostName]);
				throw new _Internal.RuntimeError({
...
++scalarPropCount;
	...
	var HostBridgeSession=(function (_super) {
		...
		HostBridgeSession.getInstanceIfHostBridgeInited=function () {
			if (HostBridge.instance) {
				if (CoreUtility.isNullOrUndefined(HostBridgeSession.s_instance) || HostBridgeSession.s_instance.m_bridge !==HostBridge.instance) {
	...
	var ClientRequestContext=(function (_super) {
		...
		ClientRequestContext._runCommon=function (functionName, requestInfo, ctxRetriever, batchMode, runBody, numCleanupAttempts, retryDelay, onBeforeRun, onCleanupSuccess, onCleanupFailure) {
			...
			var starterPromise=CoreUtility.createPromise(function (resolve, reject) { resolve(); });
			var ctx;
			var succeeded=false;
			var resultOrError;
	...
	CoreUtility.addResourceStringValues({
		...
		CustomFunctionDefintionMissing: "A property with this name that represents the function's definition must exist on Excel.CustomFunctions.",
		CustomFunctionImplementationMissing: "The property with this name on Excel.CustomFunctions that represents the function's definition must contain a 'call' property that implements the function.",
	...
	var Utility=(function (_super) {
		...
		Utility.throwIfNotLoaded=function (propertyName, fieldValue, entityName, isNull) {
			if (!isNull && CoreUtility.isUndefined(fieldValue) && propertyName.charCodeAt(0) !=Utility.s_underscoreCharCode) {
...
(function (OfficeCore) {
	...
	var AuthenticationService=(function (_super) {
		...
		AuthenticationService.prototype.getAccessToken=function (tokenParameters) {
			return _invokeMethod(this, "GetAccessToken", 1, [tokenParameters], 4 | 1, 0);
		};
...
(function (Visio) {
	...
	var Document=(function (_super) {
		...
		Object.defineProperty(Document.prototype, "onPageRenderComplete", {

			get: function () {

				var _this=this;

				if (!this.m_pageRenderComplete) {

					this.m_pageRenderComplete=new OfficeExtension.EventHandlers(

					this.context, this, "PageRenderComplete",

					{
						registerFunc: function (handlerCallback) {
							return _this.context.eventRegistration.register(6, "", handlerCallback);
						},
						unregisterFunc: function (handlerCallback) {
							return _this.context.eventRegistration.unregister(6, "", handlerCallback);
						},
						eventArgsTransformFunc: function (args) {
							return OfficeExtension.Utility._createPromiseFromResult(args.ddaBinding.Object);
						}
				...
				return this.m_pageRenderComplete;
			},
			enumerable: true,
			configurable: true
		});
